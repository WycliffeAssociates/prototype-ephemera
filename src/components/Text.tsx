...
function Text({onPhraseClick}: TextProps)
{
  ...
  verses.forEach((verse : verse, idx : number, verses : verse[]) => {
    ...
    if(hasConsumedPhraseWord)
    {
      verseWordOutput.push(<><span onClick={() => {onPhraseClick(greekWordBuffer)}} style={{textDecoration:"underline"}}>{currentPhraseWords}</span><span> </span></>)
      for(let j = 0; j < greekWordBuffer.length; j++)
      {
        greekWordBuffer.pop();
      }
      hasConsumedPhraseWord = false;
