...
function GreekWord({greekWord} : greekWordProps) {
    ...
    useEffect(() => {
        if(overwriteStyle?.fontSize != undefined) {
            let nonAdjustedFontSize = parseInt(overwriteStyle.fontSize.match(/\d+/g));
            let adjustedFontSize = 24 + (nonAdjustedFontSize - 20);
    ...
    return (
        <>
            <Grid item sm={12} xs={12}>
                <p 
                    style={{color:"blue", ...overwriteStyle, fontSize:fontSize}}
                    className="GreekWord"
                >{greekWord}</p> 
            </Grid>
        </>
